			      ...
			      long src_disp, struct var_info *dest_reg)
{
	...
	if (insn) {
		insn->src.base_reg.var_info = src_base_reg;
		insn->src.disp = src_disp;
		insn->dest.reg.var_info = dest_reg;
	}
	return insn;
}
			       ...
			       unsigned char src_shift, struct var_info *dest_reg)
{
	...
	if (insn) {
		insn->src.base_reg.var_info = src_base_reg;
		insn->src.index_reg.var_info = src_index_reg;
			      ...
			      struct var_info *dest_base_reg, long dest_disp)
{
	struct insn *insn = alloc_insn(insn_type);
	if (insn) {
		insn->src.reg.var_info = src_reg;
		insn->dest.base_reg.var_info = dest_base_reg;
			       ...
			       unsigned char dest_shift)
{
	...
	if (insn) {
		insn->src.reg.var_info = src_reg;
		insn->dest.base_reg.var_info = dest_base_reg;
		insn->dest.index_reg.var_info = dest_index_reg;
			      ...
			      struct var_info *base_reg, long disp)
{
	...
	if (insn) {
		insn->src.imm = imm;
		insn->dest.base_reg.var_info = base_reg;
		insn->dest.disp = disp;
	}
	return insn;
}
...
struct insn *reg_insn(enum insn_type insn_type, struct var_info *reg)
{
	struct insn *insn = alloc_insn(insn_type);
	if (insn) {
		insn->src.reg.var_info = reg;
		insn->dest.reg.var_info = reg;
	}
...
struct insn *reg_reg_insn(enum insn_type insn_type, struct var_info *src, struct var_info *dest)
{
	struct insn *insn = alloc_insn(insn_type);
	if (insn) {
		insn->src.reg.var_info  = src;
		insn->dest.reg.var_info = dest;
	}
	return insn;
}
