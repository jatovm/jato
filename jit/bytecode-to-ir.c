...
static int convert_nop(struct compilation_unit *cu,
...
static int convert_iconst(struct compilation_unit *cu,
					struct basic_block *bb)
{
...
static int convert_lconst(struct compilation_unit *cu,
					struct basic_block *bb)
{
...
static int convert_fconst(struct compilation_unit *cu,
					struct basic_block *bb)
{
...
static int convert_dconst(struct compilation_unit *cu,
					struct basic_block *bb)
{
...
static int convert_bipush(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return __convert_const(J_INT, (char)cu->code[1], cu->expr_stack);
}

static int convert_sipush(struct compilation_unit *cu,
					struct basic_block *bb)
{
...
static int convert_ldc(struct compilation_unit *cu,
				     struct basic_block *bb)
{
...
static int convert_ldc_w(struct compilation_unit *cu,
				       struct basic_block *bb)
{
	return __convert_ldc(&cu->cb->constant_pool,
...
static int convert_ldc2_w(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return __convert_ldc(&cu->cb->constant_pool,
			     be16_to_cpu(*(u2 *) & cu->code[1]),
...
static int convert_iload(struct compilation_unit *cu,
				       struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[1], J_INT);
}

static int convert_lload(struct compilation_unit *cu,
				       struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[1], J_LONG);
}

static int convert_fload(struct compilation_unit *cu,
				       struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[1], J_FLOAT);
}

static int convert_dload(struct compilation_unit *cu,
				       struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[1], J_DOUBLE);
}

static int convert_aload(struct compilation_unit *cu,
				       struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[1], J_REFERENCE);
}

static int convert_iload_n(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[0] - OPC_ILOAD_0, J_INT);
}

static int convert_lload_n(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[0] - OPC_LLOAD_0, J_LONG);
}

static int convert_fload_n(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[0] - OPC_FLOAD_0, J_FLOAT);
}

static int convert_dload_n(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_load(cu, bb, cu->code[0] - OPC_DLOAD_0, J_DOUBLE);
}

static int convert_aload_n(struct compilation_unit *cu,
					 struct basic_block *bb)
{
...
static int convert_iaload(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_array_load(cu, bb, J_INT);
}

static int convert_laload(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_array_load(cu, bb, J_LONG);
}

static int convert_faload(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_array_load(cu, bb, J_FLOAT);
}

static int convert_daload(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_array_load(cu, bb, J_DOUBLE);
}

static int convert_aaload(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_array_load(cu, bb, J_REFERENCE);
}

static int convert_baload(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_array_load(cu, bb, J_INT);
}

static int convert_caload(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_array_load(cu, bb, J_CHAR);
}

static int convert_saload(struct compilation_unit *cu,
					struct basic_block *bb)
{
...
static int convert_istore(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_store(cu, bb, J_INT, cu->code[1]);
}

static int convert_lstore(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_store(cu, bb, J_LONG, cu->code[1]);
}

static int convert_fstore(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_store(cu, bb, J_FLOAT, cu->code[1]);
}

static int convert_dstore(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_store(cu, bb, J_DOUBLE, cu->code[1]);
}

static int convert_astore(struct compilation_unit *cu,
					struct basic_block *bb)
{
	return convert_store(cu, bb, J_REFERENCE, cu->code[1]);
}

static int convert_istore_n(struct compilation_unit *cu,
					  struct basic_block *bb)
{
	return convert_store(cu, bb, J_INT, cu->code[0] - OPC_ISTORE_0);
}

static int convert_lstore_n(struct compilation_unit *cu,
					  struct basic_block *bb)
{
	return convert_store(cu, bb, J_LONG, cu->code[0] - OPC_LSTORE_0);
}

static int convert_fstore_n(struct compilation_unit *cu,
					  struct basic_block *bb)
{
...
static int convert_dstore_n(struct compilation_unit *cu,
					  struct basic_block *bb)
{
...
static int convert_astore_n(struct compilation_unit *cu,
					  struct basic_block *bb)
{
...
static int convert_iastore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_INT);
}

static int convert_lastore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_LONG);
}

static int convert_fastore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_FLOAT);
}

static int convert_dastore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_DOUBLE);
}

static int convert_aastore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_REFERENCE);
}

static int convert_bastore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_INT);
}

static int convert_castore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_CHAR);
}

static int convert_sastore(struct compilation_unit *cu,
					 struct basic_block *bb)
{
	return convert_array_store(cu, bb, J_SHORT);
}

static int convert_pop(struct compilation_unit *cu,
				     struct basic_block *bb)
{
...
static int convert_dup(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	void *value;
...
static int convert_dup_x1(struct compilation_unit *cu,
					struct basic_block *bb)
{
	void *value1, *value2;
...
static int convert_dup_x2(struct compilation_unit *cu,
					struct basic_block *bb)
{
...
static int convert_swap(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	void *value1, *value2;
...
static int convert_iadd(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_ADD);
}

static int convert_ladd(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_ADD);
}

static int convert_fadd(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_FLOAT, OP_ADD);
}

static int convert_dadd(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_DOUBLE, OP_ADD);
}

static int convert_isub(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_SUB);
}

static int convert_lsub(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_SUB);
}

static int convert_fsub(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_FLOAT, OP_SUB);
}

static int convert_dsub(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_DOUBLE, OP_SUB);
}

static int convert_imul(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_MUL);
}

static int convert_lmul(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_MUL);
}

static int convert_fmul(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_FLOAT, OP_MUL);
}

static int convert_dmul(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_DOUBLE, OP_MUL);
}

static int convert_idiv(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_DIV);
}

static int convert_ldiv(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_DIV);
}

static int convert_fdiv(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_FLOAT, OP_DIV);
}

static int convert_ddiv(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_DOUBLE, OP_DIV);
}

static int convert_irem(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_REM);
}

static int convert_lrem(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_REM);
}

static int convert_frem(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_FLOAT, OP_REM);
}

static int convert_drem(struct compilation_unit *cu,
				      struct basic_block *bb)
{
...
static int convert_ineg(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_unary_op(cu, bb, J_INT, OP_NEG);
}

static int convert_lneg(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_unary_op(cu, bb, J_LONG, OP_NEG);
}

static int convert_fneg(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_unary_op(cu, bb, J_FLOAT, OP_NEG);
}

static int convert_dneg(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_unary_op(cu, bb, J_DOUBLE, OP_NEG);
}

static int convert_ishl(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_SHL);
}

static int convert_lshl(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_SHL);
}

static int convert_ishr(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_SHR);
}

static int convert_lshr(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_SHR);
}

static int convert_iand(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_AND);
}

static int convert_land(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_AND);
}

static int convert_ior(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_OR);
}

static int convert_lor(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_OR);
}

static int convert_ixor(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_XOR);
}

static int convert_lxor(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_LONG, OP_XOR);
}

static int convert_iinc(struct compilation_unit *cu,
				      struct basic_block *bb)
{
...
static int convert_i2l(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_LONG);
}

static int convert_i2f(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_FLOAT);
}

static int convert_i2d(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_DOUBLE);
}

static int convert_l2i(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_INT);
}

static int convert_l2f(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_FLOAT);
}

static int convert_l2d(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_DOUBLE);
}

static int convert_f2i(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_INT);
}

static int convert_f2l(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_LONG);
}

static int convert_f2d(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_DOUBLE);
}

static int convert_d2i(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_INT);
}

static int convert_d2l(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_LONG);
}

static int convert_d2f(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_FLOAT);
}

static int convert_i2b(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_BYTE);
}

static int convert_i2c(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_CHAR);
}

static int convert_i2s(struct compilation_unit *cu,
				     struct basic_block *bb)
{
	return convert_conversion(cu, bb, J_SHORT);
}

static int convert_lcmp(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_CMP);
}

static int convert_xcmpl(struct compilation_unit *cu,
				       struct basic_block *bb)
{
	return convert_binop(cu, bb, J_INT, OP_CMPL);
}

static int convert_xcmpg(struct compilation_unit *cu,
				       struct basic_block *bb)
{
...
static int convert_ifeq(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_if(cu, bb, OP_EQ);
}

static int convert_ifne(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_if(cu, bb, OP_NE);
}

static int convert_iflt(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_if(cu, bb, OP_LT);
}

static int convert_ifge(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_if(cu, bb, OP_GE);
}

static int convert_ifgt(struct compilation_unit *cu,
				      struct basic_block *bb)
{
	return convert_if(cu, bb, OP_GT);
}

static int convert_ifle(struct compilation_unit *cu,
				      struct basic_block *bb)
{
...
static int convert_goto(struct compilation_unit *cu,
				      struct basic_block *bb)
{
...
static int convert_return(struct compilation_unit *cu,
					struct basic_block *bb)
{
