...
#define MULTI_LEVEL_OPCODES(level)                         \
    ...
    DEF_OPC(OPC_FCONST_1, level)                           \
        PUSH_##level(0x3f800000, 1);                       \
                                                           \
    DEF_OPC(OPC_FCONST_2, level)                           \
        PUSH_##level(0x40000000, 1);                       \
                                                           \
...
#define ZERO_DIVISOR_CHECK_0                               \
    ZERO_DIVISOR_CHECK(ostack[-1]);

#define ZERO_DIVISOR_CHECK_1                               \
    ZERO_DIVISOR_CHECK(cache.i.v1);

#define ZERO_DIVISOR_CHECK_2                               \
    ZERO_DIVISOR_CHECK(cache.i.v2);
...
#define BINARY_OP_1(OP)                                    \
    PUSH_0((int)*--ostack OP cache.i.v1, 1);

#define BINARY_OP_2(OP)                                    \
    PUSH_0(cache.i.v1 OP cache.i.v2, 1);

#define SHIFT_OP_0(TYPE, OP)                               \
    ostack -= 2;                                           \
    PUSH_0((TYPE)ostack[0] OP ((int)ostack[1] & 0x1f), 1);
...
#define IF_ICMP_1(COND)                                    \
    BRANCH((int)*--ostack COND cache.i.v1);

#define IF_ICMP_2(COND)                                    \
    BRANCH(cache.i.v1 COND cache.i.v2);

#define IF_0(COND)                                         \
    BRANCH((int)*--ostack COND 0);

#define IF_1(COND)                                         \
    BRANCH(cache.i.v1 COND 0);

#ifdef USE_CACHE
#define IF_2(COND)                                         \
    *ostack++ = cache.i.v1;                                \
    BRANCH(cache.i.v2 COND 0);
#else
#define IF_2(COND)                                         \
    BRANCH(cache.i.v2 COND 0);
#endif
...
#define PUSH_LONG(value, ins_len) \
    *(u8*)ostack = value;         \
    ...
    DEF_OPC_210(OPC_DCONST_1)
        PUSH_LONG(0x3ff0000000000000LL, 1);
...
{                                                        \
    TYPE *element;                                       \
    int idx = cache.i.v2;                                \
    Object *array = (Object *)cache.i.v1;                \
                                                         \
...
{                                                        \
    ...
    *ostack++ = *element;                                \
            ...
            OPC_DALOAD)
    {
        ...
        NULL_POINTER_CHECK(array);
        ARRAY_BOUNDS_CHECK(array, idx);
        PUSH_LONG(((u8 *)(INST_DATA(array) + 1))[idx], 1);
    }
...
{                                                       \
    ...
    NULL_POINTER_CHECK(array);                          \
    ARRAY_BOUNDS_CHECK(array, idx);                     \
    *(TYPE *)(((char *)INST_DATA(array)) +              \
                       (idx * sizeof(TYPE)) + 4) = val; \
    DISPATCH(0, 1);                                     \
}
#else
#define ARRAY_STORE(TYPE)                               \
{                                                       \
    ...
    *(TYPE *)(((char *)INST_DATA(array)) +              \
            ...
            OPC_DASTORE)
    {
        ...
        ((u8 *)(INST_DATA(array) + 1))[idx] = cache.l;
        DISPATCH(0, 1);
    }
#else
    DEF_OPC_012_2(
            ...
            OPC_DASTORE)
    {
        ...
        ((u8 *)(INST_DATA(array) + 1))[idx] = ((u8*)ostack)[1];
        DISPATCH(0, 1);
    }
#endif
...
#define BINARY_OP_long(OP)                               \
    cache.l = ((long long*)ostack)[-1] OP cache.l;       \
    ostack -= 2;                                         \
    DISPATCH(2, 1);
...
#define ZERO_DIVISOR_CHECK_long                          \
    ZERO_DIVISOR_CHECK(((u8*)ostack)[-1]);
#endif
...
{                                     \
    ...
    {
        double v2 = ((double *)ostack)[-1];
        double v1 = ((double *)ostack)[-2];

        ((double *)ostack)[-2] = fmod(v1, v2);
        ostack -= 2;
        DISPATCH(0, 1);
    }
...
{                                              \
    ...
    {
        long long v1 = ((long long*)ostack)[-1];
    ...
    DEF_OPC_012(OPC_LCMP)
    {
        long long v2 = ((long long*)ostack)[-1];
...
{                                                         \
    ...
    DEF_OPC_210(OPC_MULTIANEWARRAY_QUICK)
    {
        ...
        if((obj = allocMultiArray(class, dim, ostack)) == NULL)
            goto throwException;
...
invokeMethod:
{
    ...
    if(new_mb->access_flags & ACC_NATIVE) {
        ostack = (*(u4 *(*)(Class*, MethodBlock*, u4*))
