...
static void assert_empty_operand_stack(CuTest * ct, unsigned char actual)
{
				...
				char *actual, size_t count)
{
	struct operand_stack stack = OPERAND_STACK_INIT;
	struct statement *stmt =
	    convert_bytecode_to_stmts(cb, actual, count, NULL);
...
void test_convert_bipush(CuTest * ct)
{
	...
	assert_empty_operand_stack(ct, OPC_BIPUSH);
}
...
void test_convert_sipush(CuTest * ct)
{
	...
	assert_empty_operand_stack(ct, OPC_SIPUSH);
}
