...
static int stub_expand(struct buffer *buf, size_t size)
{
	nr_expand_called++;
	expand_size = size;
...
void test_should_init_buffer_with_given_ops(void)
{
	...
	buffer = __alloc_buffer(INITIAL_SIZE, &stub_ops);
	assert_int_equals(1, nr_expand_called);

	assert_int_equals(INITIAL_SIZE, expand_size);
	assert_int_equals(INITIAL_SIZE, buffer->size);
...
void test_should_expand_upon_append(void)
{
	...
	buf = __alloc_buffer(INITIAL_SIZE, &stub_ops);
