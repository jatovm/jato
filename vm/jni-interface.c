...
static jobject JNI_ToReflectedMethod(JNIEnv *env, jclass clazz, jmethodID methodID, jboolean isStatic)
{
	...
	if (vm_class_is_primitive_class(vmc) || vm_class_is_array_class(vmc))
		...
		return (jobject) vm_method_to_java_lang_reflect_method(methodID, clazz, methodID->method_index);
}
...
static jobject JNI_ToReflectedField(JNIEnv *env, jclass clazz, jfieldID fieldID, jboolean isStatic)
{
	...
	return vm_field_to_java_lang_reflect_field(fieldID, clazz, fieldID->field_index);
}
...
static const struct JNINativeInterface_ defaultJNIEnv = {
